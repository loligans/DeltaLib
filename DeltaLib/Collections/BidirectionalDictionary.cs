...
namespace DeltaLib.Collections
{
    ...
    public class BidirectionalDictionary<TFirst, TSecond> : IDictionary<TFirst, TSecond>, IReadOnlyDictionary<TFirst, TSecond>
    {
        ...
        public KeyCollection FirstKeys => _firstKeys ??= new KeyCollection(this);
        ...
        public TSecond this[TFirst firstKey]
        {
            get
            {
                ...
                if (index < 0)
                {
                    throw new KeyNotFoundException($"The given key '{firstKey}' was not present in the dictionary.");
                }
        ...
        private bool TryInsert(TFirst firstKey, TSecond secondKey, bool returnFalseOnExisting)
        {
            ...
            if (j >= 0)
            {
                ...
                {
                    throw new ArgumentException($"An item with the same key has already been added. Key: {secondKey}");
                }
            }
            else if (i >= 0)
            {
        ...
        void IDictionary<TFirst, TSecond>.Add(TFirst key, TSecond value)
        {
            if (!Add(key, value))
            {
                throw new ArgumentException($"An item with the same key has already been added. Key: {key} or {value}");
            }
        }
        ...
        [DebuggerDisplay("Count = {Count}")]
        [SuppressMessage("Microsoft.Design", "CA1034:Nested types should not be visible")]
        public sealed class KeyCollection : ICollection<TFirst>, IReadOnlyCollection<TFirst>
        {
            ...
            void ICollection<TFirst>.Add(TFirst item) => throw new NotSupportedException("Mutating a key collection derived from a dictionary is not allowed.");

            void ICollection<TFirst>.Clear() => throw new NotSupportedException("Mutating a key collection derived from a dictionary is not allowed.");
            ...
            bool ICollection<TFirst>.Remove(TFirst item) => throw new NotSupportedException("Mutating a key collection derived from a dictionary is not allowed.");
